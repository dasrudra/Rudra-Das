...
const GridTiltCard: React.FC<{ project: any; skin: ProjectSkin; index: number }> = ({ project, skin, index }) => {
  ...
  return (
    <motion.div
      ...
      className={`group flex flex-col justify-between bg-navy-900/10 border ${skin.borderColor} rounded-[28px] overflow-hidden p-6 hover:bg-navy-900/20 transition-all duration-500 relative`}
      style={{
    ...
    >
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between">
          <span className={`px-3 py-1 rounded-full bg-white/5 border border-white/10 ${skin.accentText} text-[10px] font-mono font-semibold tracking-wider uppercase`}>
            {project.domain || "SaaS"}
          </span>
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full animate-pulse" style={{ backgroundColor: skin.themeColor }} />
            <span className="text-[10px] text-white/40 font-mono uppercase">{skin.labelText.split(' ')[0]}</span>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <h3 className="text-xl font-bold text-white group-hover:text-accent-primary transition-colors flex items-center gap-1.5">
            {project.title}
          </h3>
