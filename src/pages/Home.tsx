            ...
            {projects.slice(0, 4).map((project, i) => {
              ...
              return (
                ...
                >
                  <TiltContainer
                    maxTilt={6}
                    glowColor={`${projectColor}25`}
                    glareIntensity={0.15}
                    className="glass-card p-6 rounded-[28px] overflow-hidden h-full flex flex-col justify-between transition-all duration-500 group-hover:translate-y-[-8px] group-hover:shadow-[0_20px_40px_rgba(0,0,0,0.4)] border border-white/5 hover:border-white/10"
                  >
                    <div className="space-y-5">
                      {/* Top Header: Domain Badge and Glow Indicator */}
                      <div className="flex items-center justify-between">
                        <span className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-muted-slate text-[10px] font-mono font-semibold tracking-wider uppercase">
                          {project.domain || "SaaS"}
                        </span>
                        <div className="flex items-center gap-2">
                          <span className="w-2 h-2 rounded-full animate-pulse" style={{ backgroundColor: projectColor }} />
                          <span className="text-[10px] text-white/40 font-mono uppercase">DEPLOYED</span>
                        </div>
                      </div>

                      {/* Image Block with Zoom on Card Hover */}
                      ...
                      <div className="space-y-3">
                        {/* Title */}
                        <h3 className="text-xl md:text-2xl font-bold text-white group-hover:text-accent-primary transition-colors duration-300 leading-tight">
                          {project.title}
                        </h3>

                        {/* Business-focused Description */}
                        <p className="text-muted-slate text-sm leading-relaxed min-h-[3rem]">
                          {project.description}
                        </p>

                        {/* Technology Badges (5-7 core technologies) */}
                        <div className="flex flex-wrap gap-1.5 pt-1">
                          {project.tech.map(t => (
                            <span key={t} className="text-[9px] font-medium tracking-wide text-white/70 bg-white/5 border border-white/10 px-2 py-0.5 rounded-md hover:bg-white/10 transition-colors">
                              {t}
                            </span>
                          ))}
                        </div>
                    ...
                    {/* Elegant Three-Button Footer */}
                    <div className="grid grid-cols-3 gap-2.5 pt-6 mt-6 border-t border-white/5">
                      {/* Live Demo Button */}
                      <a 
                        href={project.liveLink || project.link}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl text-center text-[10px] font-bold uppercase tracking-widest text-white bg-accent-primary/10 border border-accent-primary/30 hover:bg-accent-primary hover:text-navy-950 transition-all duration-300 transform active:scale-95 shadow-[0_0_0_rgba(16,185,129,0)] hover:shadow-[0_4px_12px_rgba(16,185,129,0.25)]"
                      >
